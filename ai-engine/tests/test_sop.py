...
def test_generates_steps_with_human_titles():
    ...
    events = [
        Event(
            id="1", user_id="u", kind="navigation", application="Zendesk",
            url="/tickets", occurred_at=base,
        ),
        Event(
            id="2", user_id="u", kind="click", application="Zendesk",
            target="Apply T1 macro", occurred_at=base + timedelta(seconds=10),
        ),
        Event(
            id="3", user_id="u", kind="form_submit", application="Zendesk",
            target="Reply", occurred_at=base + timedelta(seconds=20),
        ),
    ]
